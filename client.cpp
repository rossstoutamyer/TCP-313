...
void patient_thread_function(string filename, int patientNum, int numData, int mVal, BoundedBuffer* buf, TCPRequestChannel* channel, int w){
...
void worker_thread_function(int mVal, string filename, BoundedBuffer* buf, BoundedBuffer* response, TCPRequestChannel* channel){
...
int main(int argc, char *argv[])
{
    ...
    while((opt = getopt(argc, argv, "n:p:w:b:m:f:h:a:r:")) != -1) {
        switch (opt) {
            case 'n':
                n = atoi(optarg);
                break;
    ...
    TCPRequestChannel** workers = new TCPRequestChannel*[w];
    for (int i = 0; i < w; ++i) {
	    TCPRequestChannel* newChan = new TCPRequestChannel(host, port);
