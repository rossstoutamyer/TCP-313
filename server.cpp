...
void process_newchannel_request (TCPRequestChannel *_channel){
	...
	TCPRequestChannel *data_channel = new TCPRequestChannel (new_channel_name, "");
...
void handle_process_loop(TCPRequestChannel *channel){
	...
	while (true){
		...
		if (nbytes < 0){
			...
			cerr << "Could not read anything in channel " << endl;
			break;
		}
...
int main(int argc, char *argv[]){
	...
	while(true) {
		socklength = sizeof(client_info);
		int currSocket = accept(control_channel->getfd(), (struct sockaddr*) & client_info, &socklength);
		if (currSocket == -1) {
			perror("Could not accept");
			continue;
		}
		TCPRequestChannel *chan = new TCPRequestChannel(currSocket);
		thread hpl (handle_process_loop, chan);
		hpl.detach();
	}
